...
LOG_FILE = os.path.expanduser("~/polybot/bot_90c.log")
...
ROI_HALT_STATE_FILE = os.path.expanduser("~/polybot/trading_halt_state_90c.json")
...
DAILY_TRADE_SIZE_FILE = os.path.expanduser("~/.polybot_90c_daily_trade_size.json")
...
BOT_ID = "CHATGPT-90C"
ACTIVITY_LOG_FILE = os.path.expanduser("~/activity_log_90c.jsonl")
...
def save_trades():
    try:
        with open("trades_smart_90c.json", "w") as f:
...
def main():
    ...
    try:
        ...
        if window_state:
            trades_log.append(window_state)
        save_trades()
        print("Trades saved to trades_smart_90c.json")
